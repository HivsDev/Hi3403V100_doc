...
-   **[功能简介](#ZH-CN_TOPIC_0000002457881041)**  

-   **[ISP功能框图](#ZH-CN_TOPIC_0000002457841029)**  

-   **[各模块简介](#ZH-CN_TOPIC_0000002424202362)**  
...
-   **[录像机应用图像调优概述](#ZH-CN_TOPIC_0000002457880933)**  
...
-   **[线性模式图像质量调优](#ZH-CN_TOPIC_0000002457840933)**  

-   **[WDR模式图像质量调优](#ZH-CN_TOPIC_0000002424362214)**  
...
-   Sensor和镜头的标定工作
    ...
    **图 2**  Sensor和镜头标定的流程图<a name="fig1589155619710"></a>  
    ...
    -   Sensor的静态坏点标定：Sensor的静态坏点主要受Sensor的工艺有关，其中包括亮点和暗点，静态坏点的标定受Sensor的分辨率影响，例如消费级Sensor 包括多个分辨率，如4K@30fps、1080p@120fps、720p@240fps，不同的分辨率的静态坏点表需要重新标定；标定的过程需要分别针对亮点和暗点进行标定，得到亮点表和暗点表，再将亮点表和暗点表进行合并，可以得到整个坏点表，具体标定的流程方法请参考“[DPC](#ZH-CN_TOPIC_0000002457881037)“章节。
...
1.  调整Gamma参数，Gamma参数是影响图像对比度的基本参数，以实际静物场景为例，通过调整Gamma参数达到画面亮区部分的解析率卡和暗区部分的玩偶细节都不发生损失，且画面获得比较好的对比度；[图11](#_Ref515959136)中红色框图部分为Gamma曲线所影响的暗区细节和亮区细节。
...
4.  YUV Sharpen的调试准则主要将图像的纹理细节和边缘锐度调到合适，以实验室静物场景为例，YUV Sharpen需要将图像在经过3DNR之前将静物场景的草垫子、狮子等纹理细节锐化出来，[图14](#_Ref500231916)中红色框图部分为YUV Sharpen锐化出来的纹理细节。同时还需要将桌子和斜方格等大边锐化出来，[图15](#_Ref500231919)中红色框图部分YUV Sharpen锐化出来的大边。YUV Sharpen的参数需要根据ISO进行联动，确保在实验室静物场景中不同照度下，YUV Sharpen参数调试合理。
    ...
    **图 15**  静物场景ISO100  YUV Sharpen锐化出来的大边<a name="_Ref500231919"></a>  
    ...
    因此，BayerSharpen、YUV Sharpen以及3DNR三者需要在实验室静物场景根据不同的ISO进行反复联合调试，最终达到噪声和清晰度的平衡为合理，YUV Sharpen的具体调试方法请参考"[Sharpen](#ZH-CN_TOPIC_0000002457881141)”章节；

5.  DPC去动态坏点强度只需要在照度稍低下以去除动态坏点为准，照度稍好的情况下，建议DPC去动态坏点的强度调试为0，DPC的具体调试方法请参考"[DPC](#ZH-CN_TOPIC_0000002457881037)”章节；
6.  3DNR作为整个清晰度的调优的关键环节，主要包括时域滤波器的调优和空域滤波器的调优，优先调节时域的动静判决的阈值即将雨点噪声抑制为合适和时域的绝对强度保证画面静止区域安静，再调节动静判决的空域滤波器来抑制画面运动区域的跟随噪声，最后调节纯空域滤波器抑制画面的整体颗粒感，3DNR调试的标准是达到静止画面噪声安静且清晰度满足要求，运动区域的噪声得到比较好的抑制且运动拖尾能够被合理控制，[图16](#_Ref500232007)为某sensor 典型低照度的3DNR去噪效果图，其中红框圈出来的关注维度包括车辆的跟随噪声抑制维度、人物身后的运动拖尾维度以及静止画面的亮噪声和色噪声的抑制维度。3DNR具体调试方法请参考《SSxxxVxxx 3DNR参数配置说明》。
...
-   **[WDR背光场景提升小脸亮度应用场景调试指南](#ZH-CN_TOPIC_0000002424362142)**  

-   **[WDR交通强光抑制应用场景调试指南](#ZH-CN_TOPIC_0000002424362178)**  
...
在得到Gamma曲线的基础上，调试DRC的Asymmetry曲线，为了提升小脸的亮区，Asymmetry曲线需要提升背光的亮度，具体调试的曲线如[图5](#_Ref500232856)所示。

**图 5**  提升小脸亮度的Asymmetry曲线形状<a name="_Ref500232856"></a>  
![](figures/提升小脸亮度的Asymmetry曲线形状.png "提升小脸亮度的Asymmetry曲线形状")
...
-   **[Sharpen](#ZH-CN_TOPIC_0000002457881141)**  

-   **[Demosaic](#ZH-CN_TOPIC_0000002457841009)**  

-   **[BayerSharpen](#ZH-CN_TOPIC_0000002424362250)**  

-   **[NR](#ZH-CN_TOPIC_0000002457841025)**  

-   **[DPC](#ZH-CN_TOPIC_0000002457881037)**  

-   **[DRC](#ZH-CN_TOPIC_0000002457881045)**  

-   **[WDR](#ZH-CN_TOPIC_0000002424362078)**  

-   **[Crosstalk Removal](#ZH-CN_TOPIC_0000002457840869)**  

-   **[AntiFalse Color](#ZH-CN_TOPIC_0000002424362246)**  

-   **[Mesh Lens Shading](#ZH-CN_TOPIC_0000002457841017)**  

-   **[Gamma](#ZH-CN_TOPIC_0000002424202274)**  

-   **[White Balance](#ZH-CN_TOPIC_0000002424362106)**  

-   **[CCM](#ZH-CN_TOPIC_0000002424202278)**  

-   **[CAC](#ZH-CN_TOPIC_0000002457881105)**  

-   **[CA](#ZH-CN_TOPIC_0000002424362058)**  

-   **[Expander](#ZH-CN_TOPIC_0000002424362090)**  

-   **[Radial Crop](#ZH-CN_TOPIC_0000002424202250)**  

-   **[LDCI](#ZH-CN_TOPIC_0000002424362134)**  

-   **[Dehaze](#ZH-CN_TOPIC_0000002457840893)**  

-   **[CLUT](#ZH-CN_TOPIC_0000002457881033)**  

-   **[RGBIR](#ZH-CN_TOPIC_0000002457880969)**  

-   **[CRB](#ZH-CN_TOPIC_0000002457881101)**  

## Sharpen<a name="ZH-CN_TOPIC_0000002457881141"></a>

-   **[功能描述](#ZH-CN_TOPIC_0000002424202334)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424202226)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457880909)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424362166)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457840833)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002424362046)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424202258)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424202266)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457840977)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424202174)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424362082)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457880997)**  

-   **[噪声标定方法](#ZH-CN_TOPIC_0000002457840817)**  
...
1.  调节空域去噪滤波器。主要针对运动区域噪声进行处理，通过sfm\_thresh可以选择不同的滤波器：当sfm\_thresh=0的时候，选择sfm1滤波器处理，当sfm\_thresh=255的时候，选择sfm0滤波器处理，其他值则是sfm0和sfm1滤波器融合的模式。
    ...
    选择sfm0滤波器下，可以根据噪声水平再确定sfm0滤波器的模式sfm0\_mode, 一般而言，在中等照度以内，可以选择NORMAL模式，这个时候滤波器保边性更好，整体清晰度更高，低照度下，噪声水平较大，这个时候可以选择EXT模式，这个时候不容易出现pattern，噪声形态较好。确定滤波器以后，可以通过实际的噪声水平来控制滤波器去噪强度。相关参数包括四通道的sfm0去噪强度sfm0\_coarse\_strength\[4\]，EXT模式下细节保留比例sfm0\_ex\_detail\_prot 等。
...
1.  调节空域去噪功能。调节时需注意噪声和细节之间的平衡，因此调整去噪强度时，需要同时关注噪声和细节的表现。通过调节每一融合帧去噪强度snr\_sfm0\_wdr\_strength\[4\]\(fusion模式下为snr\_sfm0\_fusion\_strength\[4\]\)来单独控制各个融合帧的噪声水平，使得全画面的各个融合帧区域的噪声水平达到均匀一致，然后再配合滤波器其他强度参数进行调试。
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457840925)**  

-   **[坏点类型](#ZH-CN_TOPIC_0000002457840993)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457840849)**  

-   **[动态坏点校正模块](#ZH-CN_TOPIC_0000002424362130)**  

-   **[静态坏点标定](#ZH-CN_TOPIC_0000002424202322)**  
...
-   **[可以校正的坏点类型](#ZH-CN_TOPIC_0000002457841041)**  
...
|参数|描述|
|--|--|
|ot_isp_dp_dynamic_attr|enable|动态坏点校正的使能。取值范围：[0, 1]|
...
|参数|描述|
|--|--|
|ot_isp_dp_static_calibrate|enable_detect|静态坏点标定使能取值范围：[0, 1]|
...
|ot_isp_dp_static_attr|enable|静态坏点校正使能取值范围：[0, 1]|
...
-   **[动态坏点检测](#ZH-CN_TOPIC_0000002457881025)**  

-   **[坏点校正](#ZH-CN_TOPIC_0000002424362062)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457881069)**  
...
动态坏点检测的强度可以通过strength这个值进行配置，strength的值越大，动态坏点检测的强度越大（即diff值越大），检测出的坏点数目越多。strength值的选择与ISO有很大的关系，随着ISO值的增大，strength的值也应随之增大（具体在某个ISO下strength配置哪个值可以遵循“[调试步骤](#ZH-CN_TOPIC_0000002457881069)”进行调试）。
...
在自动模式下，ISO共分为了如下的\{100, 200, 400, 800, 1600, 3200, 6400, 12800, 25600, 51200, 102400,204800,409600,819200,1638400,3276800\}16档。在对应sensor的cmos.c文件中可以设置不同ISO档下strength与blend\_ratio的默认值。若连续两档ISO的对应的strength、blend\_ratio不相等，则通过线性插值的方法确定两档中间的某个ISO所对应的校正强度和融合比率。
...
3.  根据sensor特性设置start\_thresh（start\_thresh的理论可取值范围为（0, 255），注意start\_thresh不要取到0）；
...
由于亮点与暗点与各自独立进行标定的，所得的坏点信息分别保存在两个坏点表中，要得到最终的坏点信息，需要将这两个坏点表进行合并。在合并的时候，要按照坐标递增的顺序，剔除相同的坐标点，得到全部的坏点信息保存在亮坏点表 bright\_table\[OT\_ISP\_STATIC\_DP\_COUNT\_MAX\]中，并写入ISP memory。此过程中，用户需要进行的操作如下：
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457881009)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424362190)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457840973)**  

-   **[注意事项及相关问题](#ZH-CN_TOPIC_0000002457881149)**  
...
>![](public_sys-resources/icon-note.gif) **说明：** 
>自定义曲线同样要求是单调非递减的。如果配置的LUT中某点的值小于其前一点的值，该点的值将被自动置为其前一点的值，以使最终曲线符合单调非递减的条件。另外，自定义曲线实际生效的LUT比tone\_mapping\_value多一个采样点，即最大值点，其坐标固定为\(1, 1\)，不可调节，目的是强制最大亮度值在映射后仍为最大亮度值。
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457840913)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457840957)**  

-   **[WDR调试](#ZH-CN_TOPIC_0000002424362182)**  

-   **[WDR模式常见现象的调试方法](#ZH-CN_TOPIC_0000002457881089)**  

-   **[注意事项及相关问题](#ZH-CN_TOPIC_0000002457880917)**  
...
-   **[长短帧融合阈值调试](#ZH-CN_TOPIC_0000002424362146)**  

-   **[运动检测参数调试](#ZH-CN_TOPIC_0000002424202350)**  

-   **[Fusion模式阈值调试](#ZH-CN_TOPIC_0000002424202354)**  
...
-   **[AE调节时画面闪烁](#ZH-CN_TOPIC_0000002424202370)**  

-   **[图像整体偏暗或偏亮](#ZH-CN_TOPIC_0000002457840945)**  

-   **[图像亮区过曝或亮区细节不好](#ZH-CN_TOPIC_0000002424202198)**  

-   **[图像对比度差](#ZH-CN_TOPIC_0000002424362210)**  

-   **[图像噪声大](#ZH-CN_TOPIC_0000002424362170)**  

-   **[室内场景出现工频闪烁](#ZH-CN_TOPIC_0000002457881121)**  

-   **[亮背景下小脸偏暗偏朦](#ZH-CN_TOPIC_0000002424362230)**  

-   **[交通场景车灯光晕大](#ZH-CN_TOPIC_0000002457880953)**  

-   **[图像偏色](#ZH-CN_TOPIC_0000002424202382)**  
...
2.  确认DRC参数是否合理。当曝光策略为优先保证暗区效果时，WDR图像暗区应与长帧图像暗区表现相近；优先保证亮区效果时，WDR图像亮区应与短帧图像亮区表现相近。DRC模块参数调节参考 “[调试步骤](#ZH-CN_TOPIC_0000002457840973)”。
3.  确认gamma曲线是否合理。
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424362218)**  

-   **[Crosstalk参数](#ZH-CN_TOPIC_0000002424202182)**  

-   **[调试过程](#ZH-CN_TOPIC_0000002424362238)**  
...
## AntiFalse Color<a name="ZH-CN_TOPIC_0000002424362246"></a>

-   **[功能描述](#ZH-CN_TOPIC_0000002457840881)**  

-   **[AntiFalse Color参数](#ZH-CN_TOPIC_0000002457840953)**  

-   **[调试过程](#ZH-CN_TOPIC_0000002424202210)**  

-   **[注意事项及相关问题](#ZH-CN_TOPIC_0000002457840901)**  
...
### AntiFalse Color参数<a name="ZH-CN_TOPIC_0000002457840953"></a>
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457840857)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424202410)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457881153)**  
...
|参数名称|描述|
|--|--|
|ot_isp_shading_attr|en|表示LSC使能。取值范围：[0, 1]|
...
|ot_isp_shading_lut_attr|mesh_scale|增益表精度控制参数取值范围：[0, 7]|
...
Shading是一种与镜头有关的缺陷，对于每个镜头都需按照《图像质量调试工具使用指南》LSC部分所描述的步骤进行数据的采集与标定，将获取的标定结果写入cmos\_ex.h文件中或直接导入到PQTools上。具体每个参数的作用和对图像的影响可参考上表中的描述。
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457881093)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457840889)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457840929)**  

-   **[Gamma参数](#ZH-CN_TOPIC_0000002424202390)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457840965)**  

-   **[关键参数\(AWB\)](#ZH-CN_TOPIC_0000002424362038)**  

-   **[关键参数\(SPECAWB\)](#ZH-CN_TOPIC_0000002424202306)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457840989)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424202326)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457841045)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457840921)**  
...
-   **[功能概述](#ZH-CN_TOPIC_0000002457880989)**  

-   **[Local CAC](#ZH-CN_TOPIC_0000002424362014)**  

-   **[ACAC](#ZH-CN_TOPIC_0000002457840841)**  
...
-   **[关键参数](#ZH-CN_TOPIC_0000002457881129)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002424202314)**  

-   **[注意事项](#ZH-CN_TOPIC_0000002457880941)**  
...
-   若只需要校正强边缘处的紫色，则可以增加var\_threshold的值；var\_threshold的值越小，所能校正的紫边的范围越大，如[图2](#fig133951171378)所示，var\_threshold=0时所能检测的紫边范围不受强边缘条件的限制，所能检测校正的范围较宽，而当var\_threshold=200时，只能检测靠近灯管边缘的几个紫边像素能被检测校正掉。
...
-   **[关键参数](#ZH-CN_TOPIC_0000002457840949)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002457881017)**  

-   **[注意事项](#ZH-CN_TOPIC_0000002457881021)**  
...
2.  根据紫边的严重程度调整fcc\_y\_strength，fcc\_rb\_strength以及purple\_alpha。这三个参数的效果都是调试越大，去除越干净。它们的调试步骤可以这样，purple\_alpha调最大，然后再慢慢调大fcc\_y\_strength，fcc\_rb\_strength。
...
-   **[功能概述](#ZH-CN_TOPIC_0000002457881145)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424362022)**  

-   **[注意事项](#ZH-CN_TOPIC_0000002424202238)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457880925)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424362150)**  

-   **[调试过程](#ZH-CN_TOPIC_0000002424202286)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424202402)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457880961)**  

-   **[调试过程](#ZH-CN_TOPIC_0000002424202270)**  
...
|radius|拉黑的半径，到中心点的距离大于半径的区域直接拉黑。取值范围：[0, ![](figures/zh-cn_formulaimage_0000002457881317.png)]默认值![](figures/zh-cn_formulaimage_0000002424202582.png)|
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424202218)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424362174)**  

-   **[调试过程](#ZH-CN_TOPIC_0000002457881077)**  

-   **[注意事项及相关问题](#ZH-CN_TOPIC_0000002424362162)**  
...
|he_wgt|he_pos_wgt|wgt|局域直方图均衡结果与原图融合权重曲线，根据亮度控制提亮程度，曲线呈GAUSS分布，该参数为GAUSS分布最大值。取值范围：[0, 0xFF]|
...
|he_neg_wgt|wgt|局域直方图均衡结果与原图融合权重曲线，根据亮度控制变暗程度，曲线呈GAUSS分布，该参数为GAUSS分布最大值。取值范围：[0, 0xFF]|
...
-   **[对比度增强的局域程度调试](#ZH-CN_TOPIC_0000002457881029)**  

-   **[局域直方图均衡权重调试](#ZH-CN_TOPIC_0000002424362206)**  

-   **[暗区抑制参数调试](#ZH-CN_TOPIC_0000002424202254)**  

-   **[时域参数调试](#ZH-CN_TOPIC_0000002424202302)**  
...
对经过LDCI增强之后亮度降低的区域，控制方法与提升区域相似，可采用参数he\_neg\_wgt.wgt、he\_neg\_wgt.sigma、he\_neg\_wgt.mean来控制融合权重曲线形状从而控制其变暗程度。而曲线的中心值he\_neg\_wgt.mean，均方差he\_neg\_wgt.sigma和峰值he\_neg\_wgt.wgt则可以根据实际场景中哪些区域亮度需要降低来进行调节，he\_neg\_wgt.mean、he\_neg\_wgt.wgt、he\_neg\_wgt.sigma取值不同时效果如[图5](#_Ref479255366)所示，亮区对比度改善明显。
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457840825)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424202346)**  

-   **[调试过程](#ZH-CN_TOPIC_0000002457881053)**  

-   **[Dehaze参数](#ZH-CN_TOPIC_0000002457840865)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457881081)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457840961)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002424202282)**  
...
-   **[功能描述](#ZH-CN_TOPIC_0000002457881113)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002457840805)**  

-   **[参数设置注意事项](#ZH-CN_TOPIC_0000002457840897)**  

-   **[其他注意事项及相关问题](#ZH-CN_TOPIC_0000002457881061)**  
...
-   在过曝区域，通过调整exp\_ctrl和exp\_gain的值，使得过曝区域的亮度和颜色表现变得平滑。其中exp\_ctrl\[0\]的值设定的越小，过曝区域的颜色差异会越小，exp\_ctrl\[1\]的值设定越大，过曝区域和非过曝区域的差异化会越小。
...
-   **[功能描述](#ZH-CN_TOPIC_0000002424202202)**  

-   **[关键参数](#ZH-CN_TOPIC_0000002424202318)**  

-   **[调试步骤](#ZH-CN_TOPIC_0000002424362122)**  
