...
-   **[为什么执行server\_install脚本报错](#ZH-CN_TOPIC_0000002457839305)**  

-   **[MMZ和MMB分别指什么，如何配置MMZ的区域和大小](#ZH-CN_TOPIC_0000002424360494)**  

-   **[为什么会有MMB LEAK之类的打印](#ZH-CN_TOPIC_0000002424360514)**  

-   **[用fopen打开大于4G的文件失败，如何操作大于4G的文件](#ZH-CN_TOPIC_0000002457879381)**  

-   **[为什么NFS挂载不上](#ZH-CN_TOPIC_0000002424200690)**  

-   **[为什么无法烧写文件系统或者flash出现非常多的bad block](#ZH-CN_TOPIC_0000002424200702)**  

-   **[为什么无法启动文件系统，提示No init found](#ZH-CN_TOPIC_0000002424360510)**  

-   **[为什么无法启动文件系统，提示不能打开console](#ZH-CN_TOPIC_0000002457879377)**  

-   **[tftp无法使用时，需要哪些检查](#ZH-CN_TOPIC_0000002457839325)**  

-   **[为什么在服务器上执行SDK的某些编译命令或者脚本，报“找不到这个文件”或者类似的错误](#ZH-CN_TOPIC_0000002457879429)**  

-   **[制作cramfs文件系统时，报出文件大小被截为16M的警告，同时制作的文件大小不对](#ZH-CN_TOPIC_0000002457879425)**  

-   **[串口调试终端（如SecureCRT）出现显示卡住，输入无响应，但系统运行正常的情况](#ZH-CN_TOPIC_0000002424200666)**  
...
-   **[名词解释](#ZH-CN_TOPIC_0000002457839333)**  

-   **[MMZ的原理](#ZH-CN_TOPIC_0000002424200686)**  

-   **[MMZ驱动的模块参数](#ZH-CN_TOPIC_0000002424360518)**  
...
2.  bootargs配置错误，比如误把bootargs配置为：setenv bootargs ' =mem=96M console=ttyAMA0,115200 root=/dev/mtdblock2 rootfstype=yaffs2 mtdparts=nand:4M\(boot\),60M\(rootfs\),-\(others\)'之类或者bootargs中的rootfstype配置错误，比如烧写的是jffs2文件系统，bootargs却配置为yaffs2，也可能导致内核无法识别文件系统。
...
-   **[如何在单板上使用GDB](#ZH-CN_TOPIC_0000002424360562)**  

-   **[如何让gdb在调试过程中忽略信号量事件](#ZH-CN_TOPIC_0000002457839309)**  

-   **[如何让udhcpc占用更少的内存](#ZH-CN_TOPIC_0000002457879413)**  

-   **[为什么有时候udhcpc无法获得IP地址](#ZH-CN_TOPIC_0000002457839345)**  

-   **[在uboot下如何获取DDR的容量大小](#ZH-CN_TOPIC_0000002457839273)**  
...
-   **[如何使用USB键盘鼠标](#ZH-CN_TOPIC_0000002457879409)**  

-   **[为什么UDP接收或发送会丢包](#ZH-CN_TOPIC_0000002424200650)**  

-   **[GMAC网口ping不通](#ZH-CN_TOPIC_0000002424360486)**  

-   **[网络LRO和GRO介绍与使用](#ZH-CN_TOPIC_0000002457879453)**  

-   **[应用程序使用socket接口时，如何正确工作在非阻塞模式下](#ZH-CN_TOPIC_0000002424360482)**  

-   **[网线插到单板上，为什么还是会报Phy no link](#ZH-CN_TOPIC_0000002457839289)**  

-   **[为什么USB3.0口过流保护芯片去掉，会导致某些 USB3.0 U盘上电启动不识别](#ZH-CN_TOPIC_0000002424200722)**  

-   **[I2C内核态接口原子操作注意事项](#ZH-CN_TOPIC_0000002424360538)**  

-   **[SATA限速方式说明](#ZH-CN_TOPIC_0000002457879401)**  

-   **[AHB总线水线阈值问题说明](#ZH-CN_TOPIC_0000002457879461)**  
...
-   **[基本概念](#ZH-CN_TOPIC_0000002424200718)**  

-   **[开启/关闭LRO和GRO的方法](#ZH-CN_TOPIC_0000002457839313)**  
...
-   **[基本概念](#ZH-CN_TOPIC_0000002424360542)**  

-   **[IO模式设置](#ZH-CN_TOPIC_0000002457879389)**  
...
-   **[如何配置内核选项，把PCIe控制器驱动编进内核](#ZH-CN_TOPIC_0000002457839281)**  

-   **[如何配置PCIe的控制时钟](#ZH-CN_TOPIC_0000002424360558)**  

-   **[如何查看PCIe设备的BAR地址的分配信息](#ZH-CN_TOPIC_0000002424200674)**  

-   **[如何查看PCIe地址映射信息](#ZH-CN_TOPIC_0000002457879417)**  

-   **[PCIe MCC 模块的驱动插入后为何不起作用](#ZH-CN_TOPIC_0000002457839277)**  

-   **[PCIe-网卡、PCIe-sata使用注意事项](#ZH-CN_TOPIC_0000002457879449)**  

-   **[通过PCIe实现从启动失败常见的情况](#ZH-CN_TOPIC_0000002424360530)**  

-   **[为什么运行SDK视频预览的业务后，偶尔会打印“unknow irq triggered”](#ZH-CN_TOPIC_0000002457839301)**  

-   **[PCIe BAR地址，在reset后默认会映射到哪个地址，在移动窗口时，需要注意什么](#ZH-CN_TOPIC_0000002457839297)**  

-   **[在 PCIe MCC驱动不再支持主片 DMA 写从片操作](#ZH-CN_TOPIC_0000002424200646)**  

-   **[PCIe MCC 支持主设备复位从设备吗](#ZH-CN_TOPIC_0000002424360534)**  

-   **[使用某些PCIe 转SATA卡（如：marvel9215）连接sata盘读写数据，导致出现vo低带宽现象的解决方法](#ZH-CN_TOPIC_0000002424360546)**  

-   **[为何识别不到PCIE转SATA卡](#ZH-CN_TOPIC_0000002457839293)**  

-   **[PCIe作为RC时，对接其他外设卡复位电路设计](#ZH-CN_TOPIC_0000002424200642)**  
...
-   为支持超过4M的cramfs，请修改 .config 中的宏CONFIG\_BLK\_DEV\_RAM\_SIZE=65536
...
编译好主片驱动后，在 components/pcie\_mcc/out 下面会生成一个可执行文件 booter。该文件是启动从设备和复位从设备的一个简单示例，由 components/pcie\_mcc/multi\_boot/example 目录下的源码编译而成。具体的用法如下。
...
-   **[如何标记flash上的坏块](#ZH-CN_TOPIC_0000002424200698)**  

-   **[怎样把SPI flash由4线模式修改为2线模式](#ZH-CN_TOPIC_0000002424200694)**  

-   **[如何正确使用mtd-utils的nandwrite裸写工具](#ZH-CN_TOPIC_0000002424360506)**  

-   **[有些Flash器件ID不变工艺更新导致参数变化对兼容性的影响](#ZH-CN_TOPIC_0000002457879445)**  

-   **[如何在linux 4.19内核上支持SPI Nor双片选](#ZH-CN_TOPIC_0000002457839349)**  

-   **[为何当Fastboot镜像不满足block对齐时按页写入flash系统无法启动](#ZH-CN_TOPIC_0000002424360526)**  
...
-   **[NAND挂载cramfs文件系统注意事项](#ZH-CN_TOPIC_0000002424360502)**  

-   **[eMMC加载动态库注意事项](#ZH-CN_TOPIC_0000002457879457)**  
...
部分项目使用的uclibc，加载动态库或rootfs的时候，对分区有限制，需要在主分区才能正常加载，默认eMMC驱动中CONFIG\_MMC\_BLOCK\_MINORS = 8即最高支持7个主分区（额外1个表示整个磁盘），建议将动态库或者rootfs放在前面7个分区，如果需要放在第八分区及以上（如mmcblk0p8）可将CONFIG\_MMC\_BLOCK\_MINORS根据需要适当改大。
...
-   **[pid和tgid区别](#ZH-CN_TOPIC_0000002424200682)**  

-   **[普通进程和实时进程优先级](#ZH-CN_TOPIC_0000002457879465)**  

-   **[如何设置dmesg buf的大小](#ZH-CN_TOPIC_0000002457879385)**  

-   **[遇到“段错误”\(segmentation fault\)怎样生成core dump文件来进行问题分析](#ZH-CN_TOPIC_0000002424360550)**  

-   **[客户跑一个很简单的程序，top信息的loadaverage值比较大，达到2.95，而CPU的占用率比较低是否正常](#ZH-CN_TOPIC_0000002424200678)**  

-   **[GDB不能支持多线程调试的问题](#ZH-CN_TOPIC_0000002457839269)**  

-   **[关闭CONFIG\_NO\_HZ选项之后，CPU占用率升高了](#ZH-CN_TOPIC_0000002457839265)**  

-   **[为何将用户态线程的栈大小设置为16K时，线程会出现Segmentation Fault](#ZH-CN_TOPIC_0000002457839321)**  

-   **[Kswapd线程的 CPU占用率陡然升高的原因](#ZH-CN_TOPIC_0000002424200654)**  

-   **[Linux-4.19.y内核中，使用“cat /proc/vmallocinfo”命令查看vmalloc信息，地址信息显示为“0x\(\_\_\_\_ptrval\_\_\_\_\)”的原因](#ZH-CN_TOPIC_0000002424200726)**  

-   **[ARM64 flush data cache说明](#ZH-CN_TOPIC_0000002457839337)**  

-   **[I/O 密集型业务性能调优说明](#ZH-CN_TOPIC_0000002457879433)**  
...
Linux-4.4内核之前，提供了flush\_cache\_all接口，该接口通过遍历way/set，完成cache的clean和invalid操作。但之后Linux社区认为通过way/set操作cache，只能作用于本地核，和其他核的cache操作存在竞态风险，所以社区在2015年删除了flush\_cache\_all和其他相关接口。推荐使用\_\_flush\_dcache\_area接口，该接口通过虚拟地址flush cache，可以确保cache的PoC一致性。
...
## I/O 密集型业务性能调优说明<a name="ZH-CN_TOPIC_0000002457879433"></a>
...
-   **[工具链glibc库升级到2.29版本后，应用程序产生“内存空洞”问题的解决方法](#ZH-CN_TOPIC_0000002457839317)**  

## 工具链glibc库升级到2.29版本后，应用程序产生“内存空洞”问题的解决方法<a name="ZH-CN_TOPIC_0000002457839317"></a>

-   **[现象描述](#ZH-CN_TOPIC_0000002424200670)**  

-   **[原因分析](#ZH-CN_TOPIC_0000002457879397)**  

-   **[解决方法](#ZH-CN_TOPIC_0000002457879393)**  
...
默认情况下，大于等于128KB的内存分配会调用mmap/munmap，小于128KB的内存请求调用brk，但可以通过修改M\_MMAP\_THRESHOLD值来调整。
