...
-   **[ISP](#ZH-CN_TOPIC_0000002441723797)**  

-   **[Sensor相关](#ZH-CN_TOPIC_0000002441723777)**  

## ISP<a name="ZH-CN_TOPIC_0000002441723797"></a>

-   **[如何解决整体锐度不足](#ZH-CN_TOPIC_0000002441683949)**  

-   **[如何解决图像发蒙问题，提高通透性](#ZH-CN_TOPIC_0000002408284530)**  

-   **[如何解决低照度清晰度差](#ZH-CN_TOPIC_0000002441723789)**  

-   **[如何解决图像清晰度与物体边缘白边和黑边问题](#ZH-CN_TOPIC_0000002408124638)**  

-   **[如何解决图像的锯齿严重的问题](#ZH-CN_TOPIC_0000002408124646)**  

-   **[如何解决图像暗角格子问题](#ZH-CN_TOPIC_0000002441723785)**  

-   **[如何解决AE工作异常，如严重过曝或曝光不足](#ZH-CN_TOPIC_0000002408284518)**  

-   **[如何解决红外场景亮度和对比度差的问题](#ZH-CN_TOPIC_0000002441683909)**  

-   **[如何调整红外模式AE，AWB和CCM参数](#ZH-CN_TOPIC_0000002441683913)**  

-   **[如何解决低照度情况下图像出现竖条纹的问题](#ZH-CN_TOPIC_0000002408124654)**  

-   **[如何实现背光补偿](#ZH-CN_TOPIC_0000002408124658)**  

-   **[如何实现手动曝光时间，增益自动控制，或者手动增益，曝光时间自动控制](#ZH-CN_TOPIC_0000002441683929)**  

-   **[如何正确设置最大增益](#ZH-CN_TOPIC_0000002441723793)**  

-   **[如何解决AWB易受干扰问题](#ZH-CN_TOPIC_0000002408284534)**  

-   **[如何解决高温下黑电平漂移对颜色的影响](#ZH-CN_TOPIC_0000002441723781)**  

-   **[如何解决低照度亮度、对比度偏低](#ZH-CN_TOPIC_0000002441683921)**  

-   **[如何解决光源字体附近出现方向性的毛刺](#ZH-CN_TOPIC_0000002408284522)**  

-   **[如何解决AE无法控制Piris光圈](#ZH-CN_TOPIC_0000002408124662)**  

-   **[DCIris光圈手动控制可以开关，但自动模式不会关闭原因](#ZH-CN_TOPIC_0000002408124630)**  

-   **[如何解决AE闪烁](#ZH-CN_TOPIC_0000002408124650)**  

-   **[如何解决鱼眼校正放在VI模块引起的边缘噪声大的问题](#ZH-CN_TOPIC_0000002441723801)**  

-   **[如何解决开启抗闪后较亮场景仍然闪烁的问题](#ZH-CN_TOPIC_0000002441683945)**  

-   **[如何解决软光敏方案频繁切换IR\_cut的问题与使用建议](#ZH-CN_TOPIC_0000002441683941)**  

-   **[DynamicBLC使用注意事项](#ZH-CN_TOPIC_0000002441723805)**  

-   **[采集FE raw数据送到BE时黑电平配置注意事项](#ZH-CN_TOPIC_0000002408284550)**  

-   **[AE增益分开配置使用说明](#ZH-CN_TOPIC_0000002408124634)**  
...
图像清晰度不足或物体边缘（如字体或树叶或者楼宇的边缘）有白边黑边。
...
-   高温下计算R通道增益为：High\_R/G = \(K \* G + F1– F0\) /\(G + F1 – F0\)

    F0 < F1（高温下黑电平漂移）；

从sensor的光谱图可以看到，多数sensor的绿色感光强于RB两个颜色，K<1;High\_R/G - Normal\_R/G= \(1 - K\) \* \(F1 – F0\) /\(G + F1 – F0\)\>0, High\_R/G \> Normal\_R/G。红色通道增益Rgain =G/R= 1/\(R/G\)，因此High\_Rgain < Normal\_Rgain，红色通道增益偏小。同样可推出高温下蓝色通道增益偏小，因此高温下图像偏绿是必然的。

【解决】

High\_R/G - Normal\_R/G= \(1 - K\) \* \(F1 – F0\) /\(G + F1 – F0\)，G体现了亮度信息，G越大，High\_R/G与Normal\_R/G的偏差越小。物理含义是：亮度越高，对黑电平的偏移越不敏感。因此，推荐增大AWB参数的black\_level\(前默认值为0x40\)，降低黑电平漂移对增益的干扰。
...
调试BayerNR中的NrLsc模块，使能nr\_lsc\_enable，并调试Mesh-shading中的bnr\_lsc\_gain\_lut.r\_gain/bnr\_lsc\_gain\_lut.gr\_gain/bnr\_lsc\_gain\_lut.gb\_gain/bnr\_lsc\_gain\_lut.b\_gain。
...
2.  驱动需要通过cmos\_get\_ae\_comn\_default接口传递相应的参数到AE算法内部。
    -   ae\_gain\_sep\_cfg： 需要设置为True，使能增益分开配置模式。
    -   diff\_gain\_support：用于长短帧配置不同的增益，需将该模式打开。
    -   lhcg\_support： 如果是单快门双增益的sensor，需要将该配置打开。
    -   sns\_lhcg\_exp\_ratio： 单快门双增益sensor，需要将sensor内部的长短帧增益比例的配置给AE。AE在配置曝光比的时候会将lhcg\_ratio考虑进去，即长帧曝光量/短帧曝光量\*lhcg\_ratio等于最终的曝光比。
...
-   **[如何解决对接sensor时，环境亮度变化引起图像出现裂屏](#ZH-CN_TOPIC_0000002408284546)**  

-   **[如何解决不同的sensor器件实现50Hz/60Hz抗闪](#ZH-CN_TOPIC_0000002408284514)**  

-   **[如何解决sensor输出图像中间有黄圈问题](#ZH-CN_TOPIC_0000002408284510)**  

-   **[如何规避Sensor Built-in模式亮暗区域出现黄绿色彩问题](#ZH-CN_TOPIC_0000002441683933)**  

-   **[如何实现图像裁剪](#ZH-CN_TOPIC_0000002441683937)**  

-   **[如何理解接入性能](#ZH-CN_TOPIC_0000002441683925)**  
...
Slave模式sensor的模拟增益寄存器生效机制是实时生效的，当环境亮度变化时，AE刷新相关sensor寄存器，并对相关寄存器进行配置，当在图像数据的有效区（非消隐区）对其模拟增益进行配置时，模拟增益实时生效在非垂直消隐区，将会出现亮线或裂屏问题。
