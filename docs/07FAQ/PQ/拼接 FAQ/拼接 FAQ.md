...
-   **[如何理解理想的产线标定环境](#ZH-CN_TOPIC_0000002464984741)**  

-   **[如何理解双鱼眼结构产线标定环境](#ZH-CN_TOPIC_0000002464984737)**  

-   **[如何理解四路水平结构产线标定环境](#ZH-CN_TOPIC_0000002431386022)**  

-   **[如何制作彩色棋盘格产线标定环境](#ZH-CN_TOPIC_0000002431386026)**  

-   **[如何解决产线标定时棋盘格角点匹配错位问题](#ZH-CN_TOPIC_0000002431386018)**  

-   **[如何解决产线标定时无角点匹配对问题](#ZH-CN_TOPIC_0000002464864621)**  

-   **[如何优化产线标定拼接效果](#ZH-CN_TOPIC_0000002431226190)**  
...
半球面方案较为简便，即在完全理想球面的基础上进行简单裁剪，水平方向上保留180°的棋盘格球面，垂直方向上也可根据镜头FOV进行裁剪，如[图2](#fig1064814233245)所示，该示意图中只保留南纬60°至北纬60°区域，裁剪高纬度区域。
...
条状张贴时每条直接对齐有一定的公差，贴条之间可能有一定的错位，针对该错位，从技术上分析，[图6](#fig151051344185812)\(a\)中角点完整，虽然棋盘格中间有错位，不过对角点检测影响较小；而[图6](#fig151051344185812)\(b\)中角点有错位，影响标定过程中的检测准确性。
...
-   **[如何理解模型标定图基本要求](#ZH-CN_TOPIC_0000002431226186)**  

-   **[如何拍摄高质量的模型标定图](#ZH-CN_TOPIC_0000002464984733)**  

-   **[如何利用PQ Tools中tmp文件评估模型标定效果](#ZH-CN_TOPIC_0000002431386010)**  
...
-   **[内参标定图像拍摄方法](#ZH-CN_TOPIC_0000002464984729)**  

-   **[外参标定图像拍摄方法](#ZH-CN_TOPIC_0000002464864609)**  
...
|2|针对不同角度下对重叠区域全覆盖|调整距离更远（约1.8倍），使得图卡对角靠近2边拼缝，每拍一对**旋转**约30°|8|2900 mm|
...
模型标定的过程中，在PQTools的tmp文件夹中将产生三种类型的临时文件，分别为QA\_measures.txt、distance.csv及棋盘格标志的jpg图片。对这些文件的含义及使用进行说明。

【解决】

请参考"[QA\_measures.txt文件内容与使用说明](#ZH-CN_TOPIC_0000002464984725)"小节至"[棋盘格标志的jpg图片](#ZH-CN_TOPIC_0000002431386006)"小节描述。

-   **[QA\_measures.txt文件内容与使用说明](#ZH-CN_TOPIC_0000002464984725)**  

-   **[distance.csv文件内容与使用说明](#ZH-CN_TOPIC_0000002431226182)**  

-   **[棋盘格标志的jpg图片](#ZH-CN_TOPIC_0000002431386006)**  
...
图片命名为avsp\_calib\_X\_vid\_Y\_Z.jpg，其中X若为单个数字则表示该图为camera X对应的内参标定图，若X为两个数字则表示该图为这两个camera之间的外参标定图；Y为镜头后缀编号，仅在外参标定图中起作用；Z为图片后缀，以0开始编号。
...
-   **[当Pipe无法按照顺序编号时，如何正确生成及配置LUT表](#ZH-CN_TOPIC_0000002431226178)**  

-   **[LUT最佳拼接距离与视差问题](#ZH-CN_TOPIC_0000002431386014)**  

-   **[Fine Tuning调试方法](#ZH-CN_TOPIC_0000002464864613)**  
...
上式中：W为像素宽度，单位为pixel；w为sensor宽度，单位为mm；f为镜头焦距，单位为mm；b为相邻镜头间距，单位为mm；Z<sub>1</sub>/Z<sub>2</sub>为不同物体的成像距离，单位为m。其中W、w，f都为定值，视角差与b、![](figures/zh-cn_formulaimage_0000002464864685.png)成正比，所以为了减小视角差造成的错位影响，应该尽量减少相邻镜头的间距。而Z<sub>1</sub>、Z<sub>2</sub>与场景相关，是无法直接进行调控的，[表1](#table1228mcpsimp)是根据一定条件得到的不同物距的视差大小，可以看出当物距越近时，视差影响越大，所以全景相机更适用于室外大而远的场景，室内场景下因为视差影响更明显，更容易出现错位重影问题。
...
|--|--|--|--|
|Yaw = 0.05*OffsetVPitch = -0.04*OffsetH|Yaw = - 0.05*OffsetVPitch = 0.04*OffsetH|Yaw = 0.05*OffsetVPitch = -0.04*OffsetH|Yaw = - 0.05*OffsetVPitch = 0.04*OffsetH|
