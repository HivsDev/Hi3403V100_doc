...
-   **[工具概述](#ZH-CN_TOPIC_0000002441889113)**  

-   **[应用场景](#ZH-CN_TOPIC_0000002408329736)**  

-   **[烧写原理](#ZH-CN_TOPIC_0000002441768981)**  

-   **[工具与单板器件匹配关系说明](#ZH-CN_TOPIC_0000002408169824)**  

-   **[环境准备](#ZH-CN_TOPIC_0000002408169800)**  
...
-   **[适用场景](#ZH-CN_TOPIC_0000002408329724)**  

-   **[烧写步骤](#ZH-CN_TOPIC_0000002441889129)**  

-   **[制作Nand烧片器镜像](#ZH-CN_TOPIC_0000002408169740)**  

-   **[选中分区表单行点击跳转进入按地址烧写界面](#ZH-CN_TOPIC_0000002441768853)**  

## 适用场景<a name="ZH-CN_TOPIC_0000002408329724"></a>

按分区烧写功能，适用于所有的单板，不管单板上有没有boot。
...
2.  配置单板分区信息，点击浏览按钮![](figures/zh-cn_image_0000002408330452.png)，可选择已设置好的分区表信息的XML，载入到工具中，分区信息被加载，如[图2](#fig62556341)所示。
    ...
    **图 3**  编辑单板分区信息<a name="fig1406152918013"></a>  
    ...
    >-   创建完成后在切换工具时，弹出如[图5](#Figure2.6)对话框，“确定”，在弹出的对话框中选择要保存分区信息的路径，输入要保存的文件名，就会保存为xml 格式的分区信息，点击“取消”，则切换视图且不保存分区信息。注意保存分区信息的文件名后缀必须为.xml格式，否则下次载入分区信息时可能会出错而无法正确载入分区信息。信息另存为如[图6](#Figure2.7)所示。
...
-   **[适用场景](#ZH-CN_TOPIC_0000002408329660)**  

-   **[烧写步骤](#ZH-CN_TOPIC_0000002441889077)**  

-   **[上载步骤](#ZH-CN_TOPIC_0000002408329648)**  

-   **[擦除步骤](#ZH-CN_TOPIC_0000002441889041)**  
...
烧写和上载是两个逆反操作，烧写功能是将镜像文件烧录到单板上，而上载功能则是按照用户设置的起始地址和长度将这段区域的内容上载至PC。上载的具体步骤完全可以参考烧写步骤。这里列出两个和烧写步骤中不一样的地方，重复的地方不在赘述。

1.  同3.2章节[1](#li109341792610)。
2.  同3.2章节[2](#li20792205132610)。
...
4.  同3.2章节[3](#li17763211278)。
5.  单击“upload”，如果待上载区域的镜像是fastboot，kernel，ubifs等请选择Data without OOB，如果镜像是yaffs，请选择Data with OOB。如[图2](#_Ref416783742)所示。
...
擦除功能是从指定地址开始将指定长度的内容从板端擦除。擦除的步骤同烧写步骤类似，这里列出两个和烧写步骤中不一样的地方，重复的地方不在赘述。
...
-   **[适用场景](#ZH-CN_TOPIC_0000002408169760)**  

-   **[烧写步骤](#ZH-CN_TOPIC_0000002408329636)**  
...
-   **[适用场景](#ZH-CN_TOPIC_0000002441889025)**  

-   **[烧写步骤](#ZH-CN_TOPIC_0000002408329704)**  

-   **[制作烧片器镜像](#ZH-CN_TOPIC_0000002441768929)**  

-   **[上载步骤](#ZH-CN_TOPIC_0000002408169712)**  

-   **[擦除步骤](#ZH-CN_TOPIC_0000002408169768)**  
...
1.  切换到“烧写eMMC”页签，如[图1](#fig10733727164915)所示。
    ...
    **图 1**  eMMC 烧写界面<a name="fig10733727164915"></a>  
    ...
    >![](public_sys-resources/icon-note.gif) **说明：** 
    >-   切换“默认采用XML所在路径”的勾选状态，若勾选，则优先在XML路径下查找该分区文件。若不勾选，则优先采用绝对路径查找该文件，若找不到，再尝试在XML所在目录下查找该文件，该状态默认被勾选。
    >-   **XML是一个配置文件用于保存分区表信息的，可以将编辑的分区表使用工具上的Save按钮保存成一个XML文件，下次打开工具时，将XML导入进来，分区表信息就直接加载进来。**
...
1.  配置单板分区信息，点击“浏览”，可选择已设置好的分区表信息，载入工具中，如[图2](#fig19253651205117)所示界面。其中boot分区的器件类型是emmc和emmc0时，boot会被烧写进默认分区，其中emmc不会切换启动分区，emmc0会切换boot的启动分区为默认分区。boot分区的器件类型是emmc1和emmc2时，boot会被烧写进对应的boot1，boot2分区，会切换boot的启动分区为对应的boot1，boot2分区。
    ...
    **图 4**  关闭BurnTool工具时提醒是否保存分区信息界面<a name="fig0725202195510"></a>  
    ![](figures/关闭BurnTool工具时提醒是否保存分区信息界面.png "关闭BurnTool工具时提醒是否保存分区信息界面")
...
-   **[适用场景](#ZH-CN_TOPIC_0000002408329680)**  

-   **[操作步骤](#ZH-CN_TOPIC_0000002408329692)**  

## 适用场景<a name="ZH-CN_TOPIC_0000002408329680"></a>

适用场景如下：适用于SPI Flash中因存储空间较小，用户需要将多个小镜像合并为一个镜像，然后烧录到同一个block中，从而节省flash空间的场景，也适用于将其他Flash类型的镜像合并为一个镜像。

例如，有fastboot，kernel两个镜像它们分别为500K，而spi的block大小为1M，那么如果将这两个镜像当做两个分区来烧写，板端烧写命令会使用到2个block来占用，如果合并镜像后，就只需要占用单个block即可，从而省了1M的Flash空间。
...
-   **[命令设置](#ZH-CN_TOPIC_0000002441889057)**  

-   **[TFTP设置](#ZH-CN_TOPIC_0000002441768913)**  

-   **[其他设置](#ZH-CN_TOPIC_0000002441889101)**  
...
-   **[BurnTool-Debug控制台设置](#ZH-CN_TOPIC_0000002441768885)**  

-   **[检查同一网段设置](#ZH-CN_TOPIC_0000002441768921)**  
...
点击菜单栏中“窗口”-\>“首选项”进入首选项对话框，进入“BurnTool”页面，选中“Check whether the PC and Board IP addresses are in the same network segment”按钮，如[图1](#_Ref410048821)所示，表示开启在烧写前检查PC与板端IP是否在同一网段，取消选中则表示不会在烧写前检查此项。
...
-   **[BurnTool烧写中出现TFTP超时提示时的解决办法](#ZH-CN_TOPIC_0000002408169704)**  

-   **[如何使用外置的tftpd32进行镜像下载？](#ZH-CN_TOPIC_0000002441889093)**  

-   **[BurnTool烧写Fastboot分区时，工具出现报错“Failed to send start frame”的解决办法](#ZH-CN_TOPIC_0000002408329668)**  

-   **[BurnTool烧写Fastboot分区时，控制台只打印了一段“\#\#\#\#\#\#\#\#\#”后停止打印，且工具出现报错“Failed to send head frame”的解决办法](#ZH-CN_TOPIC_0000002408169788)**  

-   **[BurnTool烧写Fastboot分区时，工具出现报错“Failed to send data frame”的解决办法](#ZH-CN_TOPIC_0000002441768861)**  

-   **[BurnTool烧写Fastboot分区时，工具出现报错“Failed to execute command”的解决办法](#ZH-CN_TOPIC_0000002408169752)**  

-   **[对于文件传输方式的选择，需要注意什么？](#ZH-CN_TOPIC_0000002408169696)**  

-   **[按地址烧写界面，文件的长度要求？](#ZH-CN_TOPIC_0000002441889033)**  

-   **[遇到点击烧写，断电重启后，不开始烧写，可能原因？](#ZH-CN_TOPIC_0000002441768873)**  

-   **[串口找不到或tftp启动失败或报tftp端口被占用的原因？](#ZH-CN_TOPIC_0000002408329620)**  

-   **[烧写Nand时控制台打印pure data length和len\_incl\_bad分别是什么含义？](#ZH-CN_TOPIC_0000002408169776)**  

-   **[单板DDR Training失败的情况下工具会有什么打印？](#ZH-CN_TOPIC_0000002408329624)**  

-   **[反馈BurnTool使用过程中出现问题时需要提供什么？](#ZH-CN_TOPIC_0000002408329640)**  

-   **[如何查看是否有进程占用了tftp的69端口？](#ZH-CN_TOPIC_0000002408329684)**  

-   **[如果是64位的PC且只安装了64位版本的JRE？](#ZH-CN_TOPIC_0000002441768865)**  

-   **[非中文语言系统无法烧写中文路径镜像](#ZH-CN_TOPIC_0000002441889049)**  
...
-   检查BurnTool中网络配置是否正确，如[图2](#_Ref386011442)所示，首先检查服务器IP是否正确，若不正确点击重新加载，加载最新的PC端IP地址；然后检查子网掩码与网关是否配置正确，若正确，再检查板端IP地址是否被占用（使用ping命令，查看当前单板IP是否能够ping通，若不能则表示当前网络不通），再检查其他相关配置是否正确，将以上参数全部保证正确后再尝试重新烧写。
...
-   修改工具中TFTP参数设置，匹配当前网络环境，通过点击菜单栏上的“Window”-\>“Preferences”-\>“BurnTool”-\>“TFTP Setting”，如[图3](#_Ref386007685)所示，将“The number of consecutive packet loss”与“TFTP no response timeout”两个参数设置大一些，然后再进行烧写，查看是否正常。
...
1.  在BurnTool中正常点击烧写按钮，弹出提示框，如[图2](#_Ref386011453)所示，点击确认，开始烧写，当前就会使用外置的tftpd32进行镜像的下载，如[图3](#_Ref386011454)所示。
    ...
    **图 2**  提示内置TFTP启动失败，端口被外置tftpd32工具占用<a name="_Ref386011453"></a>  
    ![](figures/提示内置TFTP启动失败-端口被外置tftpd32工具占用.png "提示内置TFTP启动失败-端口被外置tftpd32工具占用")
...
非中文语言系统不支持带中文路径的镜像，将烧写路径修改为英文路径。
...
<td class="cellrowborder" valign="top" width="77%"><p id="p256mcpsimp"><a name="p256mcpsimp"></a><a name="p256mcpsimp"></a>NAND Flash</p>
</td>
</tr>
