...
-   **[功能介绍](#ZH-CN_TOPIC_0000002442020521)**  

-   **[方案介绍](#ZH-CN_TOPIC_0000002441980681)**  

-   **[场景介绍](#ZH-CN_TOPIC_0000002441980705)**  
...
<td class="cellrowborder" valign="top" width="79.27%" headers="mcps1.2.3.1.2 "><p id="p18139637163211"><a name="p18139637163211"></a><a name="p18139637163211"></a>通过脚本工具msprof.pyc进行Profiling数据的解析并导出相应数据。详情请参见<a href="#ZH-CN_TOPIC_0000002408421318">解析Profiling数据</a>。</p>
</td>
</tr>
</tbody>
</table>
...
-   **[msprof.pyc脚本工具介绍](#ZH-CN_TOPIC_0000002408581270)**  

-   **[一键Profiling](#ZH-CN_TOPIC_0000002442020461)**  
...
1.  参考以下步骤完成acl.json文件配置，并完成应用工程编译和运行：
    ...
    -   打开工程文件，查看调用的**svp\_acl\_init\(\)**函数，获取acl.json文件路径，具体可以参考[2](#li66486291273)
    -   修改svp\_acl\_init方法指定的acl.json文件内容，添加Profiling相关配置，格式如下所示。

        具体参数配置可以参考[3](#li1333417325516)
...
-   **[通过调用acl.json文件方式采集Profiling数据](#ZH-CN_TOPIC_0000002408581342)**  

-   **[通过调用ACL API方式采集Profiling数据](#ZH-CN_TOPIC_0000002442020433)**  
...
5.  使用命令行工具进行SSH方式将编译好的工程同步到板端进行采集。
    ...
    **表 1**  采集数据命令参数说明
    ...
    <td class="cellrowborder" valign="top" width="39.63396339633963%" headers="mcps1.2.4.1.2 "><p id="p1065417361507"><a name="p1065417361507"></a><a name="p1065417361507"></a>SSH相关配置的文件路径，使用可参考<a href="#table1631953814614">表1</a></p>
    </td>
...
-   **[解析Profiling数据](#ZH-CN_TOPIC_0000002408581254)**  

-   **[timeline数据说明](#ZH-CN_TOPIC_0000002441980593)**  

-   **[summary数据说明](#ZH-CN_TOPIC_0000002441980577)**  
...
-   **[导出timeline数据](#ZH-CN_TOPIC_0000002442020445)**  

-   **[Task Scheduler任务调度信息数据说明](#ZH-CN_TOPIC_0000002408421346)**  

-   **[ACL接口耗时数据说明](#ZH-CN_TOPIC_0000002441980605)**  
...
-   **[导出summary数据](#ZH-CN_TOPIC_0000002408421430)**  

-   **[ACL接口耗时数据说明](#ZH-CN_TOPIC_0000002441980697)**  

-   **[ACL接口调用次数及耗时数据说明](#ZH-CN_TOPIC_0000002441980665)**  

-   **[AA Core数据说明](#ZH-CN_TOPIC_0000002442020505)**  

-   **[AA Core算子调用次数及耗时数据说明](#ZH-CN_TOPIC_0000002441980621)**  
...
-   **[展示数据操作及说明](#ZH-CN_TOPIC_0000002408581226)**  
...
-   **[网络应用中的函数计算性能优化分析样例](#ZH-CN_TOPIC_0000002408421402)**  
...
2.  解析Profiling数据。
    ...
    3.  执行如下命令，解析JOBXXX目录下的Profiling数据。
        ...
        此处以通过import命令行方式解析Profiling数据为例，解析Profiling数据详细介绍请参见[解析Profiling数据](#ZH-CN_TOPIC_0000002408421318)。
...
3.  在msprof.pyc脚本所在目录继续执行如下命令，导出timeline数据。
    ...
    执行完上述命令后，会在collection-dir目录下的JOBXXX目录下生成timeline目录，不同的数据生成对应的json文件，如[图1](#fig237417213614)所示，具体内容参见[表2](#zh-cn_topic_0290106133_table972265435020)。
...
4.  在msprof.pyc脚本所在目录继续执行如下命令，导出summary数据。
    ...
    执行完上述命令后，会在collection-dir目录下生成summary目录，不同的数据（推理，系统）生成对应的csv文件，如[图2](#fig12893536173613)所示，具体内容参见[表2](#zh-cn_topic_0290119915_table2434544115813)。
